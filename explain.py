...
def get_occlusion_sensitivity(samples, model, device, class_index, occlusion_size=1):
    ...
    for idx, sample in enumerate(samples):
        ...
        for x, confidence in zip(
            range(sensitivity_map.shape[0]), target_class_predictions
        ):
...
def explain(data, model, device, class_index, occlusion_size=1):
    ...
    if len(data.shape) == 2:
        ...
        class_index = class_index.reshape(
            (1, class_index.shape[0], class_index.shape[1])
        )
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage:")
        print(
            f"  {sys.argv[0]} <model filename> <trace filename> <sensitivity map filename>"
        )
        exit()
    ...
    # Run explainer
    data = trace_array[
        start_trace_to_attack : start_trace_to_attack + number_of_traces_to_explain,
        :,
        :,
    ]
    key_index = np.argmax(log10_sum_prediction)
    class_index = aes_sbox[
        textin_array[
            start_trace_to_attack : start_trace_to_attack + number_of_traces_to_explain,
            attack_byte,
        ]
    ...
    fig = plt.figure()
    plt.title(
        f"Occlusion sensitivity for key byte {attack_byte} in trace {start_trace_to_attack}"
    )
    ax = fig.gca()
